...
| **Factions** | Per-faction manpower totals, prosperity, and total clan gold |
| **Armies** | Per-faction military power (Σ tier²×count), troop count, and war exhaustion |
| **Wars** | Active wars with exhaustion, peace pressure, and forced-peace risk level **(+ active truces)** |
| **Rebellion** | All towns sorted by rebellion risk score, with L/S/Food stats and time-to-rebel estimate |
...
| **Characters** | All live heroes and wanderers with approximate locations |
...
- Armies cache rebuilds on daily tick
...
| `B1071_OverlayController.cs` | ~3200 lines; all overlay tab logic, caching, pagination, sorting, search |
| `B1071_MapBarPanelUIExtender.cs` | UIExtenderEx prefab injection + ViewModel Mixin |
| `B1071_LedgerRowVM.cs` | ViewModel for a single 4-column overlay row |
...
All logging is centralized through the `B1071_VerboseLog` static helper class with format `[Byzantium1071][Subsystem] message`.
...
Peace events can trigger truce registration 2-3 times (two Harmony Postfixes + CampaignEvent). This is intentional — dictionary assignment is idempotent, and the redundancy ensures truces are captured even if a third-party mod skips part of the Harmony chain.
...
- `[TownInvestment] Prosperity bonus for Epicrotea: +3.00/day from 2 active patron(s).`
...
- **Mid-campaign install:** Safe — only triggers on future kingdom destructions

...
**A kingdom holding only castles could never refill its parties. This is the exact position of a player who declares independence with a single castle.**

- **Root cause:** the castle supply chain transfers everything above the local trickle from the nearest same-faction town. `FindNearestSameFactionTown` scans every town in the world with no distance cutoff, so a `null` return does not mean "the nearest town is far away" — it means the faction owns **zero** towns. That castle was then pinned at `CastleMinimumDailyRegen` (1/day) permanently, while every established kingdom recruited normally.
- **Second root cause (the worse one):** `GetDailyRegen` adds the depleted-pool emergency bonus (`DepletedRegenBonusAtZero`, +2/day below 15% fill) *after* the castle floor, so it lands inside `supplyRequest`. With no town to honour the request it was silently discarded. A cut-off castle was therefore denied the emergency top-up every other settlement in the game receives when it runs dry: a near-empty castle in a kingdom that owns towns recovers at 3/day, the same castle in a town-less realm recovered at 1/day.
- **Fix, part 1:** when the faction owns no town at all, the castle falls back on its own bound villages, raising `CastleVillageLevyPercent` (new, default **100**) of what it asked the supply chain for. At 100 the request is recovered in full, including the emergency bonus. `GetDailyRegen` has already weighted that figure by those villages' hearths, so it is village-derived rather than manpower from nowhere.
- **Fix, part 2:** a cut-off castle is floored at `CastleCutOffDailyRegen` (new, default **4**/day) rather than the ordinary `CastleMinimumDailyRegen` (1/day). The levy alone did not solve the problem: it only operates on `supplyRequest`, which is non-zero only while the pool sits below the 15% emergency threshold, so it bought roughly four days of a two-hundred-day recovery. Measured against the shipped defaults, a median castle pool of 221 refills in ~55 days at 4/day versus ~220 days at 1/day.
- **`FindNearestSameFactionTown` is now resolved every tick**, not only when `supplyRequest > 0`. The cut-off floor has to apply on days the castle asks for nothing, and "is this castle cut off" is precisely the question of whether that lookup returns null.
- **Scope:** all six starting kingdoms own towns, so not one of the game's 67 castles takes this branch at campaign start. It activates only for a realm reduced to castles alone — an independence player, a minor faction holding a castle, or a kingdom beaten down to its castles. Kingdom-wide manpower balance is unchanged: a normal kingdom holds ~10,000 manpower and regains ~25/day, exactly as before.
- The normal supply chain resumes the moment the faction takes a town. Nothing changes for a faction that already owns one.
- Same edge-case class as the v1.0.2.4 slave-economy fix: a code path that quietly assumed every faction owns a town.

### Change — Recruiting Abroad Is No Longer Free

**Reported by players as the reason the manpower system reads as a player *advantage* rather than a constraint.**

Manpower pools are per settlement, and nothing stopped a lord recruiting anywhere in Calradia. The player's effective manpower budget was therefore the entire map, while AI lords stay in their own realm and drain only their own pools. Combined with near-zero player casualties, enemy losses were permanent while the player refilled from the enemy's own villages.

- **At war — blocked.** No lord may recruit in a settlement belonging to a faction their realm is at war with. In practice this closes the enemy *village* route; hostile towns and castles cannot be entered anyway. New setting `Block recruiting in enemy realms` (default ON).
- **At peace — gold premium.** Recruiting in a realm that is not your own costs more gold: you are outbidding the local lord for men who owe him service and owe you nothing. New setting `Foreign recruitment cost preset` (0–3, default 2). The preset is an `AddFactor` on the same `ExplainedNumber` as the tier factor, and `ExplainedNumber` sums its factors rather than compounding them, so `final = base × (1 + tierFactor + foreignFactor)`. The stated ×2 is therefore against the **base** price, not against what the player already pays: under the default Moderate hire preset the effective premium is ×1.9 on a T1 village recruit, ×1.6 on a T3 and ×1.2 on a T6, because the tier factor dominates at the top end. Intended — foreign recruiting in practice means village volunteers, and a full doubling of an already tier-inflated T6 (3000g → 6000g) would have removed the option rather than priced it.
- **Manpower cost is deliberately unchanged.** Manpower belongs to the *settlement*, not to the recruiter, so charging a foreigner extra manpower would drain the host settlement faster and punish that kingdom for the visitor's behaviour — the wrong party pays. Gold belongs to the recruiter, so gold is the correct lever. It also bites the "player is too rich by the midgame" complaint.
- **Exempt only with no kingdom *and* no fief.** That is the early-game player, for whom all of Calradia would otherwise be foreign. A landless **vassal or mercenary is not exempt**: swearing to a kingdom makes that kingdom your realm and its wars your wars, so every other realm is abroad whether or not you hold a fief. Exempting on fief alone would have left the gate bypassable by simply never accepting one.
- **Tavern mercenaries and caravan guards are exempt from both rules, everywhere.** The premium exists because a settlement's volunteers owe service to their own lord and you are outbidding him for them. A wandering mercenary owes nobody anything, so there is no local lord to outbid and no reason his price should depend on whose town he happens to be drinking in. Detected on `CharacterObject.Occupation` (`Mercenary` / `CaravanGuard`, against `Soldier` for every settlement volunteer) rather than on call path — vanilla prices a tavern hire at three separate call sites and only one of them passes through `ApplyInternal`, so a path flag would have left the displayed price and the AI's affordability pre-check disagreeing with the actual charge. Caravan parties are excluded a second time on `MobileParty.IsCaravan`: a caravan trades abroad by design and must not be taxed for being where it belongs.
- Applies to AI lords on the same terms.

### Change — Crushed Rebellions Now Meet Vanilla's Fate

**Reported: 40+ mercenary clans by day 800.**

- **Root cause:** rescuing a crushed rebel clan meant marking it a minor faction, because that is the only way it qualifies for Frontier Revenue. But minor faction is Bannerlord's *mercenary company* category, so every crushed rebellion left a permanent hireable company behind. There was no cap and no exit.
- **Fix:** the rebel rescue is now off by default — a rebellion that is crushed dies, exactly as in vanilla. The town returns to its previous owner and the rebel lords are gone.
- **All three entry points honour the toggle.** The rescue is reachable from settlement loss (`OnSettlementOwnerChanged`), the session-launch scan, and clan destruction (`HandleDestroyClan`, where a rebel clan whose **leader dies** arrives). The last of those tests only `EnableClanSurvival` through `IsClanEligibleForRescue`, so gating the first two alone left the setting looking switched off while companies kept appearing at a slower rate.
- **The rescue is kept intact behind `Rescue crushed rebel clans` (default OFF)**, not removed, so the old behaviour can be restored in one click.
- **Noble clans of a destroyed kingdom are still rescued either way.** This toggle only governs rebel-origin clans.
- **Existing saves:** `Clean up leftover rebel mercenary companies` (default OFF) is an opt-in cleanup for a campaign that already accumulated them. It destroys clans, so back the save up first.
  - **It asks before destroying anything.** Enabling the setting is not treated as consent — it is one checkbox in a long list and the action is irreversible. On the next campaign day the player is shown the real count from *their* save and has to agree.
  - **It disbands a few per campaign day, not all at once.** A long campaign holds 40–100 of these; removing them on a single day wipes a large slice of the map's parties simultaneously and reads as a mass extinction with no cause. The player sees the remaining count each day and a message when it finishes.
  - **The cleanup raises a bypass flag that the rescue prefix honours.** Without it Campaign++ intercepts its own `DestroyClanAction` call, rescues the clan straight back, and reports a count it never removed. Reachable in practice: a player can run the cleanup with `RescueRebelClans` still on, wanting the old behaviour but not the backlog.
  - **A returning player is told what changed.** Once per session, a save holding leftover companies with the rescue off gets a message saying the rescue is off, that the existing companies are being left alone, and where the cleanup lives — so nobody has to read a changelog to find out.
  - The selection is narrow by construction. It requires all three of: rebellion origin (a `StringId` containing `rebel_clan`, which is what still identifies these clans after normalisation cleared `IsRebelClan`, and which no vanilla clan carries), `IsMinorFaction` set (so only clans this mod itself flagged), zero settlements, and no kingdom — a company under an active mercenary contract is skipped, since destroying it mid-contract risks a dangling mercenary entry on the employing kingdom. Hand-authored minor factions such as the Ghilman and the Jawwal are structurally out of reach, and any rebel clan that went on to take a fief is left alone.
  - It runs on the daily tick rather than at session launch: `DestroyClanAction` touches kingdoms, parties and diplomacy, and driving that during load — or from inside another action's event callback — is what corrupted saves in the past.
...
- **Core Game Systems section:** Tracks whether other mods have fully replaced Campaign++'s game models (food, volunteer, militia, prosperity, etc.). Detected at campaign load. Shows "Campaign++ active" / "Replaced - auto-patched" / "[ISSUE] Features may be off" per system.
...
- **Report status staleness indicator:** MCM Summary group now shows a "Report status" row at the top — `"Partial — load any campaign"` until model checks have run, then `"Up to date"`. Driven by the new `B1071_CompatibilityChecker.ModelChecksRan` property. Hover hint explains that the mod list detection is complete but Core Game Systems have not been checked yet.
